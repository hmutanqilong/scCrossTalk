...
PlotCrossTalkNet <- function(clu_pairs = NULL, show_type = NULL, layout = "nicely", show_sig = F, edge_col = "black", 
    edge_alpha = 0.1, node_size_min = 5, node_size_max = 10, text_size = 3, text_col = "black") {
    # check clu_pairs
    ...
    if (show_type == "number") {
        ...
        if (show_sig == T) {
            clu_crosstalk_undir <- clu_crosstalk_undir[clu_crosstalk_undir$LR_number_type == "Significant", ]
    ...
    if (show_type == "score") {
        ...
        if (show_sig == T) {
            clu_crosstalk_undir <- clu_crosstalk_undir[clu_crosstalk_undir$LR_score_type == "Significant", ]
            if (nrow(clu_crosstalk_undir) == 0) {
                stop("No significant crosstalk in clu_pairs")
            }
            plot_res_con <- clu_crosstalk_undir[, c("cluster1", "cluster2", "LR_score", "cluster1")]
    ...
    ggraph(mygraph, layout = layout) + geom_edge_link(aes(edge_width = score), edge_colour = edge_col, edge_alpha = edge_alpha) + 
        geom_node_point(aes(size = size, fill = cluster), shape = 21, color = "black") + scale_size_continuous(range = c(node_size_min, 
        node_size_max)) + scale_fill_manual(values = clu_col) + geom_node_text(aes(label = name), size = text_size, color = text_col) + 
        expand_limits(x = c(-1.2, 1.2), y = c(-1.2, 1.2)) + theme_minimal() + theme(legend.position = "none", panel.grid = element_blank(), 
