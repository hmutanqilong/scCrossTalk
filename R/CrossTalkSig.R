...
#' @export CrossTalkSig
#' @examples clu_pairs <- CrossTalkSig(clu_pairs = FindPairs(object = mouse_kidney_203_Seurat,species = 'Mouse'))
...
CrossTalkSig <- function(clu_pairs = NULL, per_num = 1000, pvalue = 0.05) {
    ...
    for (i in 1:length(clu_com)) {
        ...
        clu_clu_score_per1 <- as.data.frame(matrix(colSums(clu_clu_score_per1), nrow = 1, 
            byrow = T))
