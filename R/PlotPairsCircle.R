...
PlotPairsCircle <- function(clu_pairs = NULL, show_sig = F, ligand_clu = NULL, receptor_clu = NULL) {
    ...
    if (nrow(res_pairs) == 0) {
        stop(paste0("There is no LR pairs in clu_pairs from cluster from ", ligand_clu, " to ", receptor_clu))
    }
    if (show_sig == T) {
        res_pairs <- res_pairs[res_pairs$type == "Significant", ]
        if (nrow(res_pairs) == 0) {
            stop(paste0("There is no significant LR pairs in clu_pairs from cluster from ", ligand_clu, " to ", receptor_clu))
        }
    }
