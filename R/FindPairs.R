...
#'
#' @description Find highly expressed ligands and receptors between pairwise clusters using Z score for a Seurat
#' object (>= 3.0.0) after log1p normalization, cluster analysis
#' and tSNE or Umap dimensionality reduction, followed by inferring significant
...
#' @param species Species of cells. The species must be defined. 'Human' or 'Mouse'
#' @param use_LRdb Which ligand-receptor pairs database to ues. Default is
#' 'LRdb' using CellTalkDB, namely 3,398 human LR pairs for 'Human';
#' 2,033 mouse LR pairs for 'Mouse'. See Details for more available
#' LR databases or a custom LR database.
#' @param revise_gene Whether to revise genes according to NCBI Gene symbols
#' (updated in April 28, 2020). Default is TRUE. When using a custom LR
...
#' in each cluster. Default is 0.25
#' @param p_value Include the significantly highly expressed ligands and receptors
#' with this cutoff of p value from Z score. Default is 0.05
...
FindPairs <- function(object, species = NULL, use_LRdb = "LRdb", revise_gene = T, cell_min_pct = 0.25, p_value = 0.05) {
    clu_pairs <- list()
    ...
    if (revise_gene == T) {
        ...
        cat("New data matrix includes", ncol(ndata), "cells,", nrow(ndata), "genes and", length(clu_num), "clusters",
            "\n")
    }
    clu_pairs[[1]] <- ndata
    ...
    Sys.sleep(2)
    cat(cyan(paste0("Finding highly expressed LR pairs using ", use_LRdb, ": ", nrow(LR_database), " ", species, " LR pairs"),
        "\n"))
    pb <- progress_bar$new(format = "[:bar] Finished::percent Remaining::eta", total = nrow(clu_pair), clear = FALSE,
    ...
    if (nrow(res_pairs1) > 0) {
        ...
        colnames(clu_crosstalk) <- c("cluster1_source", "cluster2_target", "clu_com", "clu1_clu2", "LR_number", "LR_score")
        ...
        clu_crosstalk$clu1_clu2 <- paste(clu_crosstalk$cluster1_source, clu_crosstalk$cluster2_target, sep = "_")
        ...
        clu_crosstalk_undir <- data.frame(clu_com = clu_crosstalk$clu_com, clu_com = clu_crosstalk$clu_com, stringsAsFactors = F)
    ...
    Sys.sleep(2)
    # Find pathways for clusters
    cat(cyan("Finding downstream pathways for LR pairs", "\n"))
    Sys.sleep(2)
    if (species == "Mouse") {
    ...
    LR_pathway <- "NA"
    if (nrow(res_pairs1) > 0) {
        ligand_pathway <- as.data.frame(matrix(0, nrow = 0, ncol = 4))
        colnames(ligand_pathway) <- c("gene", "type", "pathway", "Freq")
        ligand_gene <- unique(res_pairs1$ligand_gene_symbol)
        kegg_pathway1 <- kegg_pathway[kegg_pathway$b.gn %in% ligand_gene, ]
        for (i in 1:length(ligand_gene)) {
            ligand_gene1 <- ligand_gene[i]
            if (ligand_gene1 %in% kegg_pathway1$b.gn) {
                kegg_pathway2 <- kegg_pathway1[kegg_pathway1$b.gn == ligand_gene1, ]
                kegg_pathway2 <- kegg_pathway2$pathway
                kegg_pathway3 <- strsplit(kegg_pathway2, split = ";")
                kegg_pathway2 <- NULL
                for (j in 1:length(kegg_pathway3)) {
                  kegg_pathway2 <- c(kegg_pathway2, kegg_pathway3[[j]])
                }
                kegg_pathway2 <- as.data.frame(table(kegg_pathway2), stringsAsFactors = F)
                kegg_pathway2$gene <- ligand_gene1
                kegg_pathway2$type <- "ligand"
                kegg_pathway2 <- kegg_pathway2[, c(3, 4, 1, 2)]
                colnames(kegg_pathway2)[3] <- "pathway"
                ligand_pathway <- rbind(ligand_pathway, kegg_pathway2)
            }
        }
        receptor_pathway <- as.data.frame(matrix(0, nrow = 0, ncol = 4))
        colnames(ligand_pathway) <- c("gene", "type", "pathway", "Freq")
        receptor_gene <- unique(res_pairs1$receptor_gene_symbol)
        kegg_pathway1 <- kegg_pathway[kegg_pathway$a.gn %in% receptor_gene, ]
        for (i in 1:length(receptor_gene)) {
            receptor_gene1 <- receptor_gene[i]
            if (receptor_gene1 %in% kegg_pathway1$a.gn) {
                kegg_pathway2 <- kegg_pathway1[kegg_pathway1$a.gn == receptor_gene1, ]
                kegg_pathway2 <- kegg_pathway2$pathway
                kegg_pathway3 <- strsplit(kegg_pathway2, split = ";")
                kegg_pathway2 <- NULL
                for (j in 1:length(kegg_pathway3)) {
                ...
                kegg_pathway2 <- as.data.frame(table(kegg_pathway2), stringsAsFactors = F)
                kegg_pathway2$gene <- receptor_gene1
                kegg_pathway2$type <- "receptor"
                kegg_pathway2 <- kegg_pathway2[, c(3, 4, 1, 2)]
                colnames(kegg_pathway2)[3] <- "pathway"
                receptor_pathway <- rbind(receptor_pathway, kegg_pathway2)
            }
        }
    }
    LR_pathway <- rbind(ligand_pathway, receptor_pathway)
    cat(green("***Done***", "\n"))
    clu_pairs[[7]] <- LR_pathway
    names(clu_pairs)[7] <- "LR_pathway"
    clu_pairs[[8]] <- para
    names(clu_pairs)[8] <- "para"
    Sys.sleep(2)
    return(clu_pairs)
}
